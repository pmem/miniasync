...
static int
test_dml_dont_invalidate_cache_flag_flush_sw_path()
{
	return dml_flush(DATA_MOVER_DML_SOFTWARE, VDM_F_DONT_INVALIDATE_CACHE);
}

static int
test_dml_dont_invalidate_cache_flag_flush_hw_path()
{
	return dml_flush(DATA_MOVER_DML_HARDWARE, VDM_F_DONT_INVALIDATE_CACHE);
}
...
main(void)
{
	...
	ret = test_dml_dont_invalidate_cache_flag_flush_sw_path();
	if (ret)
		return ret;
	...
	if (util_dml_check_hw_available() == 0) {
		...
		if (ret)
			return ret;
		ret = test_dml_dont_invalidate_cache_flag_flush_hw_path();
		if (ret)
			return ret;
	} else {
