...
main(int argc, char *argv[])
{
	...
	for (i = 0; i < MAX_TEST_ENTRIES; ++i) {
		...
		if (entry == NULL)
			break;
		ASSERTeq(membuf_ptr_user_data(entry), TEST_USER_DATA);

		entries[i] = entry;
	}

	/* if this triggers, increase MAX_TEST_ENTRIES */
	ASSERTne(i, MAX_TEST_ENTRIES);
