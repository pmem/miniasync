...
data_mover_dml_translate_flags(uint64_t flags, uint64_t *dml_flags)
{
	...
	/*
	 * Due to the issue in the DSA, VDM_F_MEM_DURABLE and
	 * VDM_F_DONT_INVALIDATE_CACHE flags cannot be used in conjuction.
	 * Therefore the VDM_F_DONT_INVALIDATE_CACHE flag will be ignored
	 * if VDM_F_MEM_DURABLE flag is set.
	 */
	if ((flags & VDM_F_MEM_DURABLE) &&
			(flags & VDM_F_DONT_INVALIDATE_CACHE))
		flags = flags & (~VDM_F_DONT_INVALIDATE_CACHE);
