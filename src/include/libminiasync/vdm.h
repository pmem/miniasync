...
#define VDM_F_VALID_FLAGS	(VDM_F_MEM_DURABLE)

/*
 * vdm_is_flag_supported -- returns if the given flag combination is supported.
 */
static inline int
vdm_is_flag_supported(struct vdm *vdm, unsigned flags)
{
	return (vdm->supported_flags && flags) == flags;
}
