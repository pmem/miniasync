 ...
 *	an uninterruptible system call that will suck up all the space in
 *	the file system and could take hours to fail. To avoid this,
 *  make a crude check to see if the requested allocation is larger
 *  than the available space in the file system (minus any blocks
 *  already allocated to the file), and if so, immediately return
 *  ENOSPC. We do the check only if the offset is 0; otherwise,
 *  trying to figure out how many additional blocks are required
 *  is too complicated.
 *
