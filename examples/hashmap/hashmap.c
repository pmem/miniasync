		...
		void *arg)
{
	...
	if (hme == NULL) {
		/*
		 * Entry not found or it's already locked, entry value
		 * can't be copied
		 */
	...
	if (dest == NULL) {
		/* Allocation failed, entry value can't be copied */
		FUTURE_INIT_COMPLETE(&fut);
		goto copy_fut;
	}
...
set_state_init_for_get(void *future,
		struct future_context *hashmap_get_ctx, void *arg)
{
	struct hashmap_get_copy_data *data =
		...
		struct future_context *fini_entry_ctx, void *arg)
{
	...
	if (hme == NULL) {
		/* Entry wasn't found, no need for entry finalization */
